...
export default function CartDrawer({
  ...
  useEffect(() => {
    try {
      ...
      if (!nameValue) nameValue = 'Client SECRET FARMZ';
      if (!emailValue) emailValue = 'client-secretfarmz@secmail.co';
      
      setCustomerName(nameValue);
      setEmailAddress(emailValue);
    } catch (e) {
      setCustomerName('Client SECRET FARMZ');
      setEmailAddress('client-secretfarmz@secmail.co');
    }
  }, []);
  ...
  const validateForm = (): boolean => {
    ...
    if (!name) {
      name = 'Client SECRET FARMZ';
      setCustomerName(name);
    }
    
    let email = emailAddress.trim();
    if (!email || !email.includes('@')) {
      email = 'client-secretfarmz@secmail.co';
  ...
  const copyReceiptToClipboard = async () => {
    ...
    const txt = `💎 SECRET FARMZ — ${t('orderSuccessTitle')} ${createdOrder.id} 💎\n` +
                ...
                `Liaison sécurisée SECRET FARMZ.`;
    try {
            ...
            {step === 'checkout' && (
              ...
              >
                <div className="text-center pb-1">
                  ...
                  <p className="text-[9px] text-orange-400/80 font-mono uppercase tracking-widest mt-1">SECRET FARMZ — Liaison Sécurisée 0-Log</p>
                </div>
              ...
              >
                ...
                <div className="p-5 rounded-2xl bg-black/60 border border-white/5 text-left text-xs leading-relaxed space-y-2.5 relative font-mono shadow-xl">
                  <div className="flex justify-between items-center pb-2 border-b border-white/5">
                    ...
                    <span className="text-[8px] text-neutral-400 uppercase tracking-wider">SECRET FARMZ PRIVATE RESERVE</span>
                  </div>
